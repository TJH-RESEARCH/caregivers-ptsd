# if you just put the correlation between VG and VR, 
# then the total effect is identifiable.
# But that ignores the more complicated
# ways in which they are related.
# this is probably a second-best scenario

dag <- 
  dagify(
    DASBO ~ PTSD + Hrs + VG + VR + Inc + Edu + Sex + Age + Race,
    VR ~~ VG,
    VG ~  SES + Sex + Age + Race,
    ...
    # coords = list(
    #   x = c(SES = -.3, Mar = -.4, Fam = -.4, VR = -.2, Hrs = -.15,VG = -.2, Edu = -.2, Inc = -.07, PTSD = 0, DASBO = .16, Sex = -.1, Age = .05, Race = .12),
    #   y = c(SES = .3, Mar = -.07, Fam = .07, VR = -.07, Hrs = - .15, VG = .07, Edu = .2, Inc = .15, PTSD = 0, DASBO = 0, Sex = .25, Age = .25, Race = .35))
  )
